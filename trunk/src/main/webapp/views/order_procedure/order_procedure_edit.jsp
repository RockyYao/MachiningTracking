    ...
    <main class="main">
        <div class="card_f">
            <div class="card-block">
                <form id="mainForm" method="post" class="form-horizontal" action="${basePath}/orderProcedure/addOrderProcedure.htm" enctype="multipart/form-data">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6-f  goods_but ">
                            <input type="text" id="enName" name="enName" class="form-control"
                                   value="${orderProcedure.enName}" readonly>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6-f  goods_but ">
                            <input type="text" id="sort" name="sort" class="form-control" value="${orderProcedure.sort}"
                                   onkeyup="this.value=(this.value.match(/\d+(\.\d{0,1})?/)||[''])[0]" maxlength="50"
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6-f  goods_but ">
                            <input type="text" id="preCostTime" name="preCostTime" class="form-control" value="${orderProcedure.preCostTime}"
                                   onkeyup="this.value=(this.value.match(/\d+(\.\d{0,1})?/)||[''])[0]" maxlength="50"
                                   onafterpaste="this.value=(this.value.match(/\d+(\.\d{0,1})?/)||[''])[0]">
                        </div>
                    </div>
            ...
            <div class="card-footer" style="text-align: center">
                <button id="submitBtn" type="submit" class="btn btn-sm btn-primary" onclick="editOrderProcedure()"><i
...
<script>
    function editOrderProcedure(){
        if(check()){
            var sort = $("#sort").val();
            var flag = true;
            $.ajax({
                url:'${basePath}/orderProcedure/checkProcedureSort.htm',
                data:{'sort':sort,'orderId':'${orderId}',"id":${orderProcedure.id}},
                type:'post',
                dataType:'json',
                async:false,
            ...
            if(flag){
                $.ajax({
                    url:'${basePath}/orderProcedure/editOrderProcedure.htm',
                    data:{"orderId":"${orderId}","id":${orderProcedure.id},"sort":$("#sort").val(),
                        "preCostTime":$("#preCostTime").val()},
                    dataType:'json',
                    type:'post',
                    success:function (data) {
                        if(data.result){
