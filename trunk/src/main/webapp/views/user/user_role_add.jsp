...
<div class="app-body">
    ...
    <main class="main">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">用户角色添加</li>
        </ol>
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <strong style="font-size:17px;font-weight:700;vertical-align: -1px;margin-right:5px">☰</strong>用户角色添加
                </div>
                <div class="card-block">
                    <form id="mainForm" method="post" class="form-horizontal" action="${basePath}/user/addUserRole.htm">
                        <input type="hidden" name="userId" value="${userId}">
                        <div class="form-group row">
                            <label class="col-sm-2 form-control-label l_class" for="roleId">选择角色</label>
                            <div class="col-sm-6  goods_but ">
                                <select id="roleId" name="roleId" class="form-control">
                                    <option value="">请选择角色</option>
                                    <c:forEach items="${roleList}" var="role">
                                        <option value="${role.id}" <c:if test="${role.id eq userRole.id}">selected</c:if> >${role.name}</option>
                                    </c:forEach>
                                </select><span class="goods_icon">*</span>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="card-footer">
                    <button id="submitBtn" type="submit" class="btn btn-sm btn-primary" onclick="saveUserRole()"><i
