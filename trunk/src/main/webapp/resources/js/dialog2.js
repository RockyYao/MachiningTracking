(function(win){
    var keDialog = {
        alert:function(options){
            ...
            var opts = mix({},keDialog.methods,keDialog.defaults,options);
            var dialogDom = opts.template(opts);
            opts.center(dialogDom);
            opts._resize(dialogDom);
            opts._events(dialogDom,opts.callback);
    ...
    keDialog.methods = {
        template:function(opts){
            var dialogDom = dom("ke_dialogbox");
            if(dialogDom){
                document.body.removeChild(dialogDom);
            }else{
                dialogDom = document.createElement("div");
            }
            var html ="<div class='ke_dtitle'><i class='iconfont icon-47hone'></i>"+opts.title+"</div>"+
                "		<a href='javascript:void(0);' class='ke_dclose'><i class='iconfont icon-cancle'></i></a>"+
                "		<div class='ke_dcnt'>"+
                "			<span class='dicon'><i class='iconfont icon-"+opts.icon+"'></i></span>"+
                "			<div class='cnt'>"+opts.content+"</div>"+
                "		</div>"+
                "		<div class='ke_dbtn'>"+
                "			<a href='javascript:void(0);' class='ke_dsure'><i class='iconfont icon-sure'></i>"+opts.stext+"</a>"+
                "			<a href='javascript:void(0);' class='ke_dcancle is-"+opts.dCancel+"'><i class='iconfont icon-inforcancle'></i>"+opts.ctext+"</a>"+
                "		</div>";
            dialogDom.className = "ke_dialog";
            dialogDom.id = "ke_dialogbox";
            dialogDom.innerHTML = html;
            //创建阴影层
            var overDom = document.createElement("div");
            overDom.className = "ke_overlay";
            document.body.appendChild(dialogDom);
            document.body.appendChild(overDom);
            return dialogDom;
        },
        center:function(dialogDom){//居中dialog
			var winHeight,winWidth,nl,nt;
            ...
            nl = (winWidth - dialogDom.offsetWidth)/2;
            nt = (winHeight - dialogDom.offsetHeight)/2;
        ...
        _resize :function(dialogDom){
            ...
            window.onresize = function(){
                obj.center(dialogDom);
            };
        },
        _events:function(dialogDom,callback){//按钮事件
            //找到三个按钮对象
            var sureDom = domClass(dialogDom,"ke_dsure")[0];
            var cancleDom = domClass(dialogDom,"ke_dcancle")[0];
            var closeDom = domClass(dialogDom,"ke_dclose")[0];

            //确定按钮事件
            sureDom.onclick = function(){
                kekeAnimate(dialogDom,{opacity:0,top:-400},function(){
                	$('.ke_overlay').remove();
                    // document.body.removeChild(this.nextElementSibling);//关闭阴影层
                    document.body.removeChild(this);//关闭弹出层
            ...
            closeDom.onclick = function(){
                kekeAnimate(dialogDom,{opacity:0,left:-400},function(){
                    document.body.removeChild(this.nextElementSibling);//关闭阴影层
                    document.body.removeChild(this);//关闭弹出层
                    if(callback)callback(false);
                });
    ...
    win.kalert = keDialog.alert;
    //win.kconfrim = keDialog.confirm;
    //win.kprompt = keDialog.prompt;
    //win.kiframe = keDialog.iframe;
